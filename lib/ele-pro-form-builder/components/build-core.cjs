...
function generateItemKey() {
  return `f${core.uuid(15, 36).toLowerCase()}`;
}
function generateUniqueItemKey(items, keys) {
  let itemKey = generateItemKey();
  while (true) {
    if (!core.findTree(items, (d) => d.key === itemKey) && (!keys || !keys.includes(itemKey))) {
      break;
    }
...
function generateBuildFormItem(type, formItems, componentData, itemTypeData$1) {
  ...
  if (!item.type || item.type === "div" || util.isContainerType(item, itemTypeData$1) && !(typeData == null ? void 0 : typeData.renderLabelText)) {
    item.label = "";
  }
  if (item.type === "table") {
    item.label = "";
