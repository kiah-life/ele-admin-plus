...
<template>
  ...
  >
        ...
        <template #body>
          <div class="ele-pro-form-builder-body-wrapper">
            <BodyHeader
              ...
              :formProps="formProps"
              :headerTools="headerRightToolNames"
    ...
    />
    <ComponentPicker
      v-model="showAddChildPicker"
      :addParentFormItemId="addParentFormItemId"
      :formItems="formProps?.items"
      :componentData="componentData"
      :itemTypeData="itemTypeData"
      @updateItems="handleUpdateItems"
...
<script lang="ts" setup>
  import { ref, computed, watch, useModel } from 'vue';
  import { findTree, eachTree, omit } from '../utils/core';
  import { useMobile, useMobileDevice } from '../utils/hook';
  import { setValue } from '../ele-pro-form/util';
  import type { ProFormItemKey, ProFormItemProps } from '../ele-pro-form/types';
  ...
  /** 是否折叠左侧边栏 */
  const leftSideCollapse = ref<boolean>(mobile.value);

  /** 是否折叠右侧边栏 */
  const rightSideCollapse = ref<boolean>(mobile.value);
  ...
  /** 历史记录数据 */
  const historyDataList = ref<TemplateFormConfig[]>([]);

  /** 恢复记录数据 */
  const redoDataList = ref<TemplateFormConfig[]>([]);
  ...
  /** 要添加子级的父级表单项 key */
  const addParentFormItemId = ref<ProFormItemKey>();
  ...
  /** 更新表单属性值 */
  const setFormPropValue = (field: string, value: any) => {
    if (!formProps.value) {
  ...
  const updateFormItemProp = (
    ...
    if (item) {
      if (!field) {
        const temp = omit(value as ProFormItemProps, ['key', 'children']);
  ...
  /** 导入 */
  const handleImport = (data: TemplateFormConfig) => {
    storeHistory();
  ...
  /** 打开添加子级的组件选择弹窗 */
  const handleOpenComponentPicker = (formItemId: string) => {
